...
class App:
    def __init__(self, master):
        ...
        self.title_label = tk.Label(master, text="Peptide position finder in protein")
        ...
        self.protein_browse_button = tk.Button(
            self.protein_ref_frame, text="Browse", command=lambda: self.browse(self.protein_entry)
        )
        self.peptide_frame = tk.LabelFrame(master, text="Select path of peptide sequence file")
        self.peptide_entry = tk.Entry(self.peptide_frame, width=80)
        self.peptide_browse_button = tk.Button(
            self.peptide_frame, text="Browse", command=lambda: self.browse(self.peptide_entry)
        )
        ...
        self.spin_box = tk.Spinbox(self.button_frame, from_=1, to=1000)
        self.spin_box.insert(0, "80")

        self.sping_box_label = tk.Label(self.button_frame, text="Line length:")
        ...
        self.sping_box_label.grid(row=0, column=0, padx=10, pady=20)
        self.spin_box.grid(row=0, column=1, padx=10, pady=20)
        self.analyze_button.grid(row=1, column=0, columnspan=2, padx=20, pady=20)
    ...
    def generate_alignment_report(self, dataset, type_of_experiment, alignment_obj, html_report_config):
    ...
    def find_peptide_in_protein_seq(self) -> None:
       ...
       """
        ...
        with open(load_file(r"html_files\index.html"), "r") as html:
            ...
            self.generate_alignment_report(alignment_obj.group_names, "group", alignment_obj, html_report_config)
            self.generate_alignment_report(alignment_obj.sample_names, "sample", alignment_obj, html_report_config)
